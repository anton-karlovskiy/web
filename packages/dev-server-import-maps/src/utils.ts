...
import { NormalizedInjectSetting, InjectSetting, ImportMapData } from './importMapsPlugin';
import { ParsedImportMap } from '@import-maps/resolve';
import { getAttribute, predicates, query } from '@web/dev-server-core/dist/dom5';

export const IMPORT_MAP_PARAM = 'wtr-import-map';
...
export function shouldInject(path: string, setting: NormalizedInjectSetting) {
  const included = !setting.include || setting.include(path);
  const excluded = setting.exclude && setting.exclude(path);
